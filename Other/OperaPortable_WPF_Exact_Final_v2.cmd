...
$ProxyArg = "-bind-address 127.0.0.1:18084","-verbosity 30" # параметры прокси
$ShowConsole = $true # показывать консоль при старте для контроля ошибок

# ============================================================================
# ТЕМА PROGRESSBAR
# ============================================================================
# Для другого браузера меняйте только значения в этом блоке.
$ProgressTheme = @{
   WindowBackground   = '#0B5162'
   TextColor          = '#D7E7F7'
   ProgressBackground = '#0B5162'
   ProgressForeground = '#0078D7'
   ProgressBorder     = '#D7E7F7'
}
...
Function Test-DotNetFramework45 {
   try {
      $release = 0

      # Основной вариант: 64-bit view.
      try {
         $release = [int](Get-ItemPropertyValue `
            'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' `
            -Name Release -ErrorAction Stop)
      } catch {}

      # Fallback для 32-bit PowerShell на 64-bit Windows.
      ...
      if ($release -lt 378389) {
         Write-Host '.NET Framework 4.5 or newer is required.' @red
         Show-Balloon -Message '.NET Framework 4.5 or newer is required.' -MessageType 'Error'
         return $false
      }
...
Function Get-LatestVersion {
   if (-not (Test-DotNetFramework45)) {exit}
...
# Форма для прогресс-бара (WPF/XAML)
Function Get-ProgressForm ([string]$Title) {
   try {
      foreach ($assembly in @('PresentationFramework','PresentationCore','WindowsBase','System.Xaml')) {
         try {
            if (-not ([AppDomain]::CurrentDomain.GetAssemblies() |
      ...
      # Размеры и расположение элементов намеренно повторяют старый WinForms-вариант:
      # Form 317x150, Label 10,20 / 280x20, ProgressBar 10,50 / 280x20.
      $xaml = @'
...
'@
      ...
      $progressForm = [Windows.Markup.XamlReader]::Load($reader)

      $progressForm.Title = $Title
      try { $progressForm.Icon = Get-IconHandle } catch {}

      $brushConverter = New-Object Windows.Media.BrushConverter
      $rootGrid = $progressForm.FindName('RootGrid')
      $progressLabel = $progressForm.FindName('ProgressLabel')
      $progressArea = $progressForm.FindName('ProgressArea')
      $progressTrack = $progressForm.FindName('ProgressTrack')
      $progressIndicator = $progressForm.FindName('ProgressIndicator')

      $rootGrid.Background = $brushConverter.ConvertFromString($ProgressTheme.WindowBackground)
      $progressLabel.Foreground = $brushConverter.ConvertFromString($ProgressTheme.TextColor)
      $progressTrack.Background = $brushConverter.ConvertFromString($ProgressTheme.ProgressBackground)
      $progressTrack.BorderBrush = $brushConverter.ConvertFromString($ProgressTheme.ProgressBorder)
      $progressIndicator.Background = $brushConverter.ConvertFromString($ProgressTheme.ProgressForeground)

      # Храним состояние отмены в Tag. Закрытие окна = отмена.
      $progressForm.Tag = $false
      $progressForm.Add_Closing({
         $progressForm.Tag = $true
      })

      return $progressForm,$progressArea,$progressLabel,$progressIndicator
   } catch {
      Write-Host "$_.Exception.Message" @red
      return $null
   }
...
# Потоковая загрузка с WPF ProgressBar.
# Загрузка выполняется в отдельном powershell.exe, поэтому UI-поток свободен
# и WPF может нормально обрабатывать перерисовку даже на Windows 10 LTSC.
Function Download-File ([string]$Source,[string]$Dest,[string]$Text,[boolean]$Pbar) {
   if (-not $Pbar) {
      try {
         $response = Make-NetRequest -Url $Source
         if (-not $response) {throw 'No HTTP response'}
   ...
   $tmpDest = "$Dest.tmp_$([IO.Path]::GetRandomFileName())"
   $metaFile = "$tmpDest.meta"
   $workerFile = Join-Path $env:TEMP ("opera_http_" + [IO.Path]::GetRandomFileName() + '.ps1')
   $progressForm = $null
   $timer = $null
   $worker = $null
   $result = $false
   ...
   try {
      # Worker получает URL, целевой tmp-файл и proxy через параметры.
      $workerCode = @'
param(
...
try {
   ...
   if ($contentLength -and $contentLength -gt 0) {
      [IO.File]::WriteAllText($MetaFile, [string]$contentLength, (New-Object System.Text.UTF8Encoding $false))
   }
...
'@
      ...
      $proxy = $null
      if ($UseProxy -eq 2 -and $ProxyArg) {
         $proxy = ($ProxyArg -Split ' ')[1]
      ...
      if ($proxy) {$workerArgs += @('-Proxy',$proxy)}

      # Start-Process получает уже экранированную командную строку.
      $escapedArgs = foreach ($arg in $workerArgs) {
         '"' + ($arg -replace '(?<!^)"','\"') + '"'
      }
      ...
      $formData = Get-ProgressForm -Title $BrowserName
      if (-not $formData) {throw 'Unable to create WPF progress window'}
      $progressForm,$progressArea,$progressLabel,$progressIndicator = $formData

      $progressLabel.Text = "$Text:   0,00 MB   of   calculating..."
      $progressForm.Show()

      # Размер ProgressArea после layout нужен для точного расчёта ширины индикатора.
      $progressForm.UpdateLayout()
      $trackWidth = [double]$progressArea.ActualWidth - 2
      if ($trackWidth -lt 1) {$trackWidth = 278}

      $contentLength = 0L
      $script:DownloadUiState = @{
         LastLength = -1L
         TotalLength = 0L
         Started = Get-Date
         Result = $false
      ...
      $timer.Add_Tick({
         if (-not $worker -or $worker.HasExited) {
            $timer.Stop()
            if ($worker -and $worker.ExitCode -eq 0 -and [IO.File]::Exists($tmpDest)) {
               $script:DownloadUiState.Result = $true
            } else {
               $script:DownloadUiState.Result = $false
            }
            $progressForm.Close()
            return
         }

         if ($progressForm.Tag -eq $true) {
            $script:DownloadUiState.Canceled = $true
            $timer.Stop()
         ...
         if ($script:DownloadUiState.TotalLength -le 0 -and [IO.File]::Exists($metaFile)) {
            try {
               $script:DownloadUiState.TotalLength = [int64]([IO.File]::ReadAllText($metaFile).Trim())
            } catch {}
         }

         $length = 0L
         try {
            if ([IO.File]::Exists($tmpDest)) {
               $length = (Get-Item -LiteralPath $tmpDest -ErrorAction Stop).Length
            }
         } catch {}

         if ($length -ne $script:DownloadUiState.LastLength) {
            $script:DownloadUiState.LastLength = $length
            $mb = $length / 1MB

            # Пока Content-Length неизвестен, используем indeterminate-аналог:
            # индикатор остаётся видимым на минимальной ширине.
            if ($script:DownloadUiState.TotalLength -le 0) {
               $progressIndicator.Width = [Math]::Min(40,[Math]::Max(4,$trackWidth * 0.10))
               $progressLabel.Text = "$Text:   $('{0:N2} MB' -f $mb)   of   calculating..."
            } else {
               $percent = [Math]::Min(100,[Math]::Max(0,($length * 100.0) / $script:DownloadUiState.TotalLength))
               $progressIndicator.Width = [Math]::Max(1, $trackWidth * ($percent / 100.0))
               $progressLabel.Text = "$Text:   $('{0:N2} MB' -f $mb)   of   $('{0:N2} MB' -f ($script:DownloadUiState.TotalLength / 1MB))"
            }
         }

         if (((Get-Date) - $script:DownloadUiState.Started).TotalSeconds -gt 900) {
            $script:DownloadUiState.Canceled = $true
            $timer.Stop()
            try {$worker.Kill()} catch {}
            $progressForm.Close()
      ...
      $script:DownloadUiState.TotalLength = 0L
      $timer.Start()
      [void]$progressForm.ShowDialog()
      $timer.Stop()

      # Закрытие окна пользователем означает отмену. Закрытие таймером после
      # нормального завершения worker отменой не считается.
      if ($progressForm.Tag -eq $true -and -not $script:DownloadUiState.Result) {
         $script:DownloadUiState.Canceled = $true
         if ($worker -and -not $worker.HasExited) {try {$worker.Kill()} catch {}}
      }

      $canceled = [bool]$script:DownloadUiState.Canceled
      $result = [bool]$script:DownloadUiState.Result

      if ($canceled) {
         Show-Balloon -Message 'Download canceled' -MessageType 'Warning'
         return $false
      }
      if (-not $result) {throw 'Download worker failed'}

      if (-not [IO.File]::Exists($tmpDest)) {throw 'Downloaded file is missing'}

      if ([IO.File]::Exists($Dest)) {[IO.File]::Delete($Dest)}
      [IO.File]::Move($tmpDest,$Dest)
      return $true
   } catch {
      Write-Host "$_.Exception.Message" @red
      return $false
      ...
      if ($worker -and -not $worker.HasExited) {try {$worker.Kill()} catch {}}
      if ($progressForm) {try {$progressForm.Close()} catch {}}
      if ([IO.File]::Exists($workerFile)) {try {[IO.File]::Delete($workerFile)} catch {}}
      if ([IO.File]::Exists($metaFile)) {try {[IO.File]::Delete($metaFile)} catch {}}
      if ([IO.File]::Exists($tmpDest)) {try {[IO.File]::Delete($tmpDest)} catch {}}
   }
}
